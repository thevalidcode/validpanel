...
const storeType = [
  { type: 'Shop', logo: FaShoppingBag, desc: 'Traditional e-commerce store', selected: true },
  {type: 'Social Media Store', logo: FaShareAlt, desc: 'Social commerce platform', selected: false}
]
...
const CreateStorePage = () => {
  ...
  const handlePaymentSelection = (e: MouseEvent<HTMLButtonElement>, idx:number) => {
    e.preventDefault();
    ...
    <div className="w-full flex flex-col gap-10 lg:flex-row justify-between px-5 border-t border-vgrey-border">
      <div className=" w-full sm:max-w-[672px] space-y-5 py-8 ">
        <div className="flex flex-col justify-between items-center gap-7">
          ...
          <p className="text-vgrey-text">Set up a new store by filling in the details below.</p>
          <div className="p-5 lg:p-8 w-full shadow-2xl rounded-2xl flex flex-col gap-5 border border-vgrey-border ">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 ">
              {
                stores.map((store, i) => (
                  <button key={store.type} onClick={()=>handleSelectStore(i)} className={`rounded-xl text-sm lg:text-base outline-none font-semibold border h-[144px] flex flex-col items-center justify-center px-5 gap-5 ${store.selected ? 'border-primary bg-primary/5 text-primary' : 'border-vgrey-border text-vgrey-text btn-custom'}  `}>
                    <store.logo className={store.selected ? 'text-primary fill-primary' : 'text-shadow-vgrey-text fill-vgrey-text'} />
                    <span className={`inline-block`}> {store.type} </span>
                    <span className="inline-block text-vgrey-text"> {store.desc} </span>
                  </button>
                ))
              }
            </div>
            <form className="grid grid-cols-1 gap-5">
              <div className="space-y-1 w-full">
                <label htmlFor="storeName" className="font-semibold text-sm block">Store Name</label>
                <TextInput placeholder="Enter your store name" />
              </div>
              <div className="space-y-1 w-full relative">
                <label htmlFor="domain" className="font-semibold text-sm block">Custom Subdomain</label>
                <TextInput placeholder="mystore" />
                <span className="block absolute bottom-1.5 right-0.5 bg-vgrey-border p-4 ">.validpanel.com</span>
              </div>
              <div className="flex flex-col gap-1">
                <p className="font-semibold text-sm block lg:text-base">Store Logo</p>
                {/* Dropzone */}
                <div
              ...
              <div className="min-h-21 w-full flex flex-col gap-2 justify-between ">
                <p className="font-semibold text-sm block lg:text-base">Brand Color</p>
                <div className="flex flex-wrap gap-3">
                  {
                    brandColor.map((col) => (
                      <button onClick={e=>handleColor(e)} key={col.color} style={{backgroundColor: col.color}} className={`w-12 h-12 rounded-xl`} />
                    ))
                  }
                </div>
              </div>
              <div className="min-h-21 w-full flex flex-col gap-2 justify-between ">
                <p className="font-semibold text-sm block lg:text-base">Payment Method</p>
                <div className="grid grid-cols-2 sm:flex flex-wrap gap-3">
                  {
                    payments.map((pay, i) => (
                      <button onClick={(e)=>handlePaymentSelection(e, i)} key={pay.name} className={`flex gap-2 justify-center py-3 px-4 rounded-xl border items-center ${pay.selected? 'border-primary bg-primary/5':'border-vgrey-border btn-custom'}`}>
                        {
                          pay.logo ? (
          ...
          <div className="flex w-full overflow-x-auto gap-3">
            {
              products.map((prod) => (
                <div key={prod.name} className="rounded-lg p-3 bg-[#F3F4F6]">
                  ...
                  <p className="font-semibold text-xs text-primary"> {prod.price} </p>
                </div>
              ))
            }
          </div>
