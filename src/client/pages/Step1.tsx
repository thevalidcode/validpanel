...
import {
  getOnboardingDraft,
  setOnboardingDraft,
  initializeDraftFromCouponParam,
} from "@/utils/onboarding.utils";
import type { StoreType } from "@/types";
import OnboardingLayout from "../components/onboarding/OnboardingLayout";
import { FaArrowRight } from "react-icons/fa";
import {
  CouponProgressIndicator,
  OnboardingStepGuide,
  OnboardingProgressHint,
} from "../components/onboarding/CouponProgressIndicator";
...
const StepOne: React.FC = () => {
  ...
  // Also capture coupon from URL params on first load
  useEffect(() => {
    const draft = getOnboardingDraft();

    // Initialize coupon from URL if present
    if (couponCode && !draft?.couponCode) {
  ...
  const handleNext = (): void => {
    ...
    const nextUrl = couponCode
      ? `/onboarding/step2?coupon=${couponCode}`
      : "/onboarding/step2";
  ...
  return (
    ...
    >
      <OnboardingStepGuide
        ...
        showCouponHint={!!draft?.couponCode && draft.couponSource === "URL"}
      >
        <div className="space-y-6 sm:px-4 px-2 pb-10 pt-2">
          ...
          <div className="flex md:flex-row flex-col gap-6">
            ...
            >
              ...
              <p className="text-gray-600 text-sm mb-4 leading-relaxed">
                ...
                Create a professional storefront with product catalogs,
          ...
          {/* Progress Hint */}
          <OnboardingProgressHint currentStep={1} totalSteps={6} />
        </div>
      </OnboardingStepGuide>
    </OnboardingLayout>
  );
